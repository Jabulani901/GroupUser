...
namespace GroupUser.Controllers
{
    ...
    {
        private readonly IAccountDataAccessService _dataAccess;
        public AccountController(IAccountDataAccessService dataAccess)
        {
        ...
        public ActionResult Login(LoginView login, string ReturnUrl = "")
        {
            try
            {
                ...
                {
                    bool isValidUser = _dataAccess.ValidateUserLogin(login.EmailId,(login.Password));
                    if (isValidUser)
                    {
                        int timeout = login.RememberMe ? 15600 : 20; 
